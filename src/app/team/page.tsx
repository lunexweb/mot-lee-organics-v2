    ...
    <div className="min-h-screen bg-gray-50 overflow-x-hidden">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            { icon: Users, color: 'text-primary-600', bg: 'bg-primary-50', label: 'Direct Referrals', value: teamMembers.length },
            { icon: TrendingUp, color: 'text-green-600', bg: 'bg-green-50', label: 'Active Members', value: activeMembers },
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          {filteredMembers.length === 0 ? (
            ...
            <div>
              <div className="grid grid-cols-[1fr_auto_auto_auto] gap-3 px-4 py-2 bg-gray-50 border-b border-gray-100">
                {['Member', 'IBO', 'Joined', 'Status'].map(h => (
                  <div key={h} className="text-xs font-medium text-gray-500 uppercase tracking-wider">{h}</div>
                ))}
              </div>
              {filteredMembers.map((member) => (
                <div key={member.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-3 items-center px-4 py-3 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                  <div className="min-w-0">
